...
const INFURA_APIKEY='c15cda28875542d9828b419651ed2323';
/*let INFURA_APIKEY = ''; // Put your Infura API Key (https://infura.io)
process.stdin.once('data', function(data) {
    INFURA_APIKEY += data.toString();
});*/
...
let isAddressAvailable = (name, callback) => {
	ens.owner(`${name}.eth`).then(addr => {
        if (addr == '0x0000000000000000000000000000000000000000') {
            ens.getExpirationTime(`${name}.eth`).then(expiry => {
                callback({
                    available: true,
                    expiryTime: expiry
                });
